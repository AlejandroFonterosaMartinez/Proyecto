...
<body>
    ...
    function mostrar(fila) {
        if (editando == 0) {
            ...
            for (let i = 1; i < 8; i++) {
                ...
                elementos[i].innerHTML = "<textarea  id='textarea' cols='30' rows='10' class='texta2'>" + elementos[i]
                    .innerHTML + "</textarea>"
    ...
    function cargar() {
        ...
        $.ajax({
            ...
            .done(function(response) {
                ...
                const botonesBorrar = document.querySelectorAll(".btn-danger");
                botonesBorrar.forEach(deshBtn => deshBtn.addEventListener("click", function(e) {
                        e.preventDefault();
                        console.log('dentro de boton');
                        deshabilitar(deshBtn.parentNode);
                    })

                );
                const botonesHabilitar = document.querySelectorAll(".btn-outline-warning");
                botonesHabilitar.forEach(habBtn => habBtn.addEventListener("click", function(e) {
                        e.preventDefault();
                        console.log('dentro de boton');
                        habilitar(habBtn.parentNode);
                    })

                );
                const botonesFavoritos = document.querySelectorAll(".btn-outline-info");
                botonesFavoritos.forEach(favBtn => favBtn.addEventListener("click", function(e) {
                        e.preventDefault();
                        console.log('dentro de boton');
                        favoritosSi(favBtn.parentNode);
                    })

                );
                const botonesNofavoritos = document.querySelectorAll(".btn-outline-danger");
                botonesNofavoritos.forEach(nFavBtn => nFavBtn.addEventListener("click", function(e) {
                        e.preventDefault();
                        console.log('dentro de boton');
                        favoritosNo(nFavBtn.parentNode);
                    })
    ...
    function categoria() {
        ...
        $.ajax({
            ...
            .done(function(response) {
                ...
                const botonesBorrar = document.querySelectorAll(".btn-danger");
                botonesBorrar.forEach(deshBtn => deletBtn.addEventListener("click", function(e) {
                        e.preventDefault();
                        console.log('dentro de boton');
                        deshabilitar(deshBtn.parentNode);
                    })
    ...
    function deshabilitar(fila) {
        ...
        $.ajax({
                ...
                method: "POST",
                url: "productos_deshabilitar.php"
            })
            .done(function(response) {
                cargar();
            });
